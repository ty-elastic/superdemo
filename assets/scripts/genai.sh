...
config_o11y_ai_assistant() {
   ...
   output=$(curl -s -X POST "$elasticsearch_kibana_endpoint/internal/kibana/settings" \
      ...
      -d '{"changes":{"contextEngine": "enabled", "agentBuilder:tracing:enabled": true, "genAiSettings:tokenUsageTracking": true, "aiAssistant:preferredChatExperience": "agent", "agentBuilder:dashboardTools": true, "agentBuilder:experimentalFeatures": true, "genAiSettings:defaultAIConnector": "'$AI_CONNECTOR'"}}')
